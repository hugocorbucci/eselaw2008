...
free) if their source code is available and modifiable (maybe not in
the same repositoryt) by anyone with the required technical knowledge
without prior consentment from the original author and without any
charge (this definition is closer from the free software idea than the
open source one).

Another restriction will be that projects started and controlled by a
company do not fit in our definition of open source. The reason beside
this is that projects controlled by companies, whether they have a
public source code and accept external collaboration or not, can be
run with any software engineering methodology since the company can
enforce it to her employees. Some methodologies will work better to
attract external contributions but the company is still in control of
its own team and does not depend on external collaboration to run it.
...
involved. In such context, one cannot expect volunteers that
contribute for fun to follow a rigid heavyweight
process. % Achar uma referencia sobre lightweight vs heavyweight
...
since it shows the health of the project. Nowadays, concerns related
