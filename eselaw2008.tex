...
\begin{abstract}
  ...
  agile methods. This work identifies and expose the
  obstacles that separate those communities in order to extract the
  best of them and improve both sides with suggestions of tools and
...
around a leader is the only hierarquical structure in the group. At
first, this argument could indicate that such projects are not
candidates for the use of agile methods since some basic values seem
to be damaged. For example, the distance and diversity separating
...
some principles of the Agile Manifesto \cite{AgileManifesto}. Being
ready for changes, working with continuous feedback, delivering real
...
mergings to partially tackle software development problems.
...
necessary to first define what is understood by such words in our
work. Agile methods are defined at Section \ref{subsec:agile-def}
while the open source one, more controvertial, is given at Section
\ref{subsec:os-def}.
...
them better organized (question 22.4). All those data show open source
contributors perceive their activities both pleasurable and effective.
...
have teams with up to 5 people and 62\% work with each other over the
...
organized. Those results show that documentation is not considered to
be essential on the projects.
...
development. Finally section \ref{subsec:agile-improve-os} will
shortly present some benefits could be introduced in agile methods.
...
Reis as an invited guest. Reis is a Brazilian open source developer
...
started with Reis' presentation on how LaunchPad is developed. Three
main points were highlighted during the discussions that followed the
presentation and will be describe in the next subsection. The first
one (Section \ref{subsubsec:commiter}) describes and discusses the
role of commiter. The second one (Section \ref{subsubsec:publicity})
presents the benefits of having a transparent and public process and
the last (Section \ref{subsubsec:crossrev}) talks about cross
reviewing systems used to ensure communication and clarity of the
code.
...
are three stages in open source common hierarquy. The first level is
...
the trunk branch. Nobody except the commiter and the contributor know
about this change. The third role is the commiter one. At this level,
the transition is explicit. Contributors and commiters vouch for a
...
circle within the team until it chooses his own fixed commiters.
...
them attribute a business meaning to their results. A few initiatives
...
disencouraged to favor external comments and ideas. Those logs help
...
or flipcharts. The problem with this approach is that whiteboards are
erased and flipcharts are recycled. Even when those are persisted
...
communication bandwidth but gain on their searchability. In a short
term, it is evident that talking is more effective than writting when
...
approved by the external reviwer. They call this a cross review of API
...
API will be approved by two different developpers.
...
architectural decisions as well as ensures that she agrees on the
...
workspace. Most of the objetive metrics are related to the code base
...
tool that could improve such scenario would be web-based plugin system
...
This practice suggests that the team should gather periodically to
discuss the way the project is going. There are two issues in such
practice in open source software teams. The first one is to have all
members of the team present at the same time. The second one is to
have them interact collectivelly in a shared area placing notes on
time stating problems and good things they felt.
...
assynchronously the time line. The team leader would ocasionally
...
the same problem as the retrospective. It envolves having the team
...
newcomers at every login. It should also be possible for members to
